...
## 1. 学习定位
这份文档围绕“${topic}”展开，不是课程目录，而是一份可以直接阅读的算法讲解。学习算法时最重要的不是背名字，而是说清楚四件事：它解决什么问题、输入输出是什么、核心状态或关键变量表示什么、为什么按照这个步骤更新就能得到正确答案。
...
掌握 ${name} 的关键，是把“状态表示”和“转移公式”讲清楚。你可以用一句话记住：${isFloyd ? "Floyd 通过逐步放开可用中转点，更新任意两点之间的最短距离。" : "算法通过清晰的状态定义和有依据的转移步骤，把复杂问题拆成可执行的程序过程。"} 学完后建议自己手推一个 3 到 4 个点的小图，再写代码验证结果，这样比只看公式更稳。`;
  ...
  return `# ${title || topic || "个性化知识讲解文档"}

## 1. 学习定位
这份文档围绕“${topic}”展开，不是课程目录，而是一份讲解型学习材料。它会先解释核心概念，再说明知识点之间的关系，最后给出示例、易错点和学习建议。

## 2. 核心概念讲解
学习这个主题时，可以先抓住三个问题：它研究什么对象、解决什么问题、在课程或项目中如何使用。不要只记一个定义，而要把定义中的关键词拆开，看每个关键词对应什么条件、步骤或限制。

如果“${topic}”偏理论，要重点理解概念边界和推导过程；如果偏实践，要重点理解输入、处理流程和输出结果。一个合格的讲解文档，应该能让你知道什么时候使用这个知识点、为什么这样做、以及做错时如何定位问题。
...
这样做的好处是，你不会只停留在“背定义”，而是能看清概念在真实任务中的输入、处理过程和输出。
...
function normalizeGeneratedResources(data, demand) {
  ...
  if (doc && doc.type === "专业课程讲解文档") {
    ...
    const demandText = String(demand || "").toLowerCase();
    const genericMultimodalLeak = !/多模态|multimodal/.test(demandText) && /图片、文字、音频|不同模态|图像内容、文本描述|多模态/.test(contentText);
    const algorithmMissingCore = isAlgorithmDemand(demand) && !/状态|转移|复杂度|代码|例题|示例/.test(contentText);
    const tooShort = contentText.length < 1200 || !/常见误区|易错|例题|示例|小结|学习建议/.test(contentText) || genericMultimodalLeak || algorithmMissingCore;
    if (tooShort) {
      doc.content = buildFallbackKnowledgeDocument(demand, doc.title);
    }
  }
...
重要：专业课程讲解文档必须是“讲义正文”，不是大纲。它至少包含：学习定位、核心概念逐段解释、知识点之间的关系、示例或类比、步骤推导、常见误区/易错点、学习建议、小结。正文控制在约 1300-1700 个中文字符，目标约 1500 字；不能少到只有提纲，也不要写成长篇论文。禁止只输出课程目标和目录。
如果主题是算法、数据结构或具体算法名（例如 Floyd、Dijkstra、动态规划、最短路），知识讲解文档必须围绕该算法本身展开，必须包含：问题定义、输入输出、状态/变量含义、核心转移或关键步骤、正确性直觉、复杂度、手推例题、代码示例、易错点。禁止输出与该算法无关的多模态、前端或通用学习法内容。
