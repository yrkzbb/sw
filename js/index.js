...
function normalizeGeneratedResources(data, demand) {
  ...
  if (doc && doc.type === "专业课程讲解文档") {
    ...
    const tooShort = contentText.length < 1200 || !/常见误区|易错|例题|示例|小结|学习建议/.test(contentText);
    if (tooShort) {
      doc.content = `# ${doc.title || demand || "个性化知识讲解文档"}

## 1. 学习定位
这份文档不是课程目录，而是一份围绕“${demand || "当前学习需求"}”展开的讲解型学习材料。它会先解释核心概念，再用例子说明应用方式，最后给出易错点和学习建议。
...
学习这个主题时，建议不要只记术语，而要追问：它解决什么问题？输入和输出是什么？关键步骤如何串起来？它和相邻概念有什么区别？
...
\`\`\`text
场景：一个系统需要同时理解图片、文字、音频或视频
输入：来自不同模态的信息，例如图像内容、文本描述、语音片段
过程：提取特征、对齐语义、融合信息、生成判断或回答
输出：分类结果、解释文本、检索结果、交互反馈或生成内容
\`\`\`
...
- **概念之间迁移混乱**：相邻概念可能名字相似，但适用场景、输入输出和评价标准不同，不能机械照搬。
...
如果你希望继续深入，可以把这个主题拆成三个层次：第一层是基础定义，确保能说清楚“它是什么”；第二层是机制过程，能说清楚“它如何工作”；第三层是应用判断，能说清楚“什么时候该用它、什么时候不该用它”。当你能用自己的话完成这三层表达时，说明你已经不只是记住了概念，而是开始具备迁移应用能力。

## 8. 自检问题

1. 我能否用一句话解释这个主题的核心问题？
2. 我能否举出一个真实应用场景？
3. 我能否指出至少两个容易混淆的相邻概念？
4. 我能否设计一个小例子来验证自己的理解？

带着这些问题阅读后续资源，会比单纯浏览材料更有效。`;
    }
  }
