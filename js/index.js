...
function isPoorReadingMaterial(content) {
  ...
  return !hasConcreteSource || !hasActionableSections || genericOnly;
}
...
function normalizeGeneratedResources(data, demand) {
  ...
  if (reading) {
    ...
    if (isPoorReadingMaterial(reading.content)) {
...
3. 论文/报告/综述：前沿或技术主题至少给 2 篇具体论文、报告或 survey，写题名和 URL/DOI/arXiv；基础课程主题可给经典教材章节代替。
...
视频/动画要包含分镜、旁白、画面元素和互动提问；代码案例要包含任务说明、代码骨架或完整示例、运行/调试提示。`;
